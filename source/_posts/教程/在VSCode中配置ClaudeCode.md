...
### 下载cc插件
首先再vscode中下载插件 **Claude Code for VS Code** ，完成后在右上角会有图标 ![](在VSCode中配置ClaudeCode/image.png) ，这就是打开ClaudeCode对话的按钮，点开后会有登录认证，如果使用的不是Claude模型，那么就要准备绕过认证。
...
访问网站[cc-switch](https://github.com/farion1231/cc-switch/releases)(←点这里)，下划至**Assets**，展开全部，下载对应文件（Windows选择 **...Windows.msi** 下载），然后打开该文件进行安装**（请下载到合适的目录）**

### 接入你的模型
打开cc switch后，点击右上角的**加号**，选择你拥有的模型供应商，填入你的**API Key**，并更改模型名字，点击添加后，回到主页面选择并使用，就可以在vscode中愉快的使用AI了。
例如，如果要使用*DeepSeek-v4-pro*，我们在DeepSeek官网充值后，创建API Key，修改如下部分：
![](在VSCode中配置ClaudeCode/image-3.png)
![](在VSCode中配置ClaudeCode/image-2.png)
# 参考资料
